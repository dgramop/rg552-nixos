...
stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    ${rkbin.src}/tools/mkimage -n rk3399 -T rksd -d ddr.bin idbloader.img
    cat miniloader.bin >> idbloader.img

    # 2. uboot.img (U-Boot packed in Rockchip format)
    ${rkbin.src}/tools/loaderimage --pack --uboot ${ubootDrv}/u-boot-dtb.bin uboot.img 0x00200000
    ...
    ${rkbin.src}/tools/trust_merger --ignore-bl32 RK3399TRUST.ini
